...
export default function UserDetailsRow(props) {
    ...
    const Stat = ((props) => {
        return (
            <p className={`${props.className} ${props.warning && props.warning == true ? 'warning_text' : 'greyText'}`}>{props.value} <b>{props.type}</b></p>
        )
    });
    ...
    return (
        <div href={url.href != null ? url.href : "#"} target='_blank' rel='noreferrer' className={`user_details_row outline ${props.className} hover`}>
            <ProfilePic src={props.user.avatar}/>
            <div className='details'>
                {/* {props.user.displayName && <p className='user_details_row_user_details_name'>{props.user.displayName}</p>} */}
                {props.user.handle && <Link href={url.href != null ? url.href : "#"} target='_blank' rel='noreferrer' className='user_details_row_user_details_name'>{props.user.handle} • <span className='greyText'>{props.user.displayName}</span></Link>}
                <div className='user_stats'>
                    {/* <Stat className="subtext" value={commas_to_number(NaN)} type="reports against user"/> */}
                    {/* <p className='greyText'>•</p> */}
                    <ToolTip text={ratio}><Stat className="subtext" warning={ratio_warning} value={ratio} type={`F/F score`}/></ToolTip>
                    
                </div>
                <p className='subtext greyText'>Joined: <span style={{ color: joined.raw.days <= 7 && joined.raw.months == 0 ? "#fc5378" : null }}>{joined.string} ago</span> ({joined_date.getDate()} {number_to_month(joined_date.getMonth())} {joined_date.getFullYear()})</p>
                {props.user.description && <p className='bio greyText'>{props.user.description}</p>}
                ...
                <div className='user_actions'>
                    ...
                    {/* <Button_with_icon icon="/icons/earth-outline.svg" className="outline hover">Community name</Button_with_icon> */}
                </div>
            </div>
        </div>
    )
